...
library(betareg)
dfmod<-data.frame(cbind(daten[!(is.na(daten$aver)|rownames(daten)%in%outl),c("passed","attempted","complex","experience","course")],coefs))
...
model1 <- betareg(prop ~ complex+S1+S2+S3+S4|complex+S1+S2+S3+S4, data=dfmod[,-c(1,2)], link="logit",link.phi = "log", type="ML")
#summary(model1)
```
...
x=seq(0,1,0.001)
plot(x, dbetar(x,mu[1],phi[1]), type="l", ylim=c(0,15), ylab="f(y)", xlab="y", axes=FALSE, col=NA)
axis(1)
axis(2, las=2)
ind = labl = rn = NULL
for (i in 1:nrow(dfmod)){
