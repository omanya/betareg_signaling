...
# pca
X = as.matrix(df[!rownames(df)%in%outl,])
pcmod<-prcomp(X)
pcs<-c(1:5)
...
loadings_base = pcmod$rotation

n <- nrow(X)
p <- ncol(X)
k <- 4

loadings_loo_rot <- array(NA, dim = c(p, k, n))

for (i in 1:n) {
  X_loo <- X[-i, ]
  pca_loo <- prcomp(X_loo, center = FALSE, scale. = FALSE)
  L_loo <- pca_loo$rotation[, 1:k]

  # Procrustes alignment to baseline loadings
  proc <- procrustes(loadings_base, L_loo, scale = FALSE)
  loadings_loo_rot[, , i] <- proc$Yrot[,1:k]
}

```

```{r tabPC}
stability <- matrix(NA, n, k)

for (i in 1:n) {
  for (j in 1:k) {
    stability[i, j] <- cor(loadings_base[, j], loadings_loo_rot[, j, i])
  }
}


#mean(stability)
tabPC = cbind(Strategy = paste0("S",1:k), Mean = round(apply(stability,2,mean),4), SD = round(apply(stability,2,sd),4))

caption<-"Stability of the extracted strategies using leave-one-out approach and correlations of the resulting perturbed components with the original ones, extracted using the whole dataset. The first column presents the average correlations and the second column reports their standard deviations. "

 knitr::kable(tabPC, align = "llll",
               col.names = c("Strategy","Mean correlation","Standard deviation of the correlation"),
               row.names = FALSE,
               digits = 4,
              escape=F,
               caption = caption
...
rq_res <- residuals(model1, type = "quantile")

# Basic diagnostic plots
par(mfrow = c(1, 2))
plot(rq_res, ylab = "Randomized quantile residuals",
     main = "Residuals vs. Index")
abline(h = 0, lty = 2)

qqnorm(rq_res, main = "QQ-Plot of Quantile Residuals")
qqline(rq_res, lty = 2)
...
plot(cd, ylab = "Cook's Distance", main = "Cook's Distance")
abline(h = 4 / length(cd), col = "red", lty = 2)

# Identify influential observations
indr = which(cd > 4 / length(cd))
...
```{r, include = FALSE}
model1_r <- betareg(aver ~ complex+S1+S2+S3+S4|complex+S1+S2+S3+S4, data=dfmod[-indr,], link="probit",link.phi = "log", type="ML")
summary(model1_r)
```

```{r, include =T}
model0 <- betareg(aver~complex+S1+S2+S3+S4, data=dfmod, link="logit")
print(paste0("Delta AIC: ", AIC(model1, model0)[2,2] - AIC(model1, model0)[1,2]))
```
